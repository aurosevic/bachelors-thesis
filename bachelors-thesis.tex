...
%			\hspace{0.7cm}
	...
	\todo[inline]{Razmisli o reorganizaciji poglavlja}
	Na kraju, pripremljeni podaci se dele u grupe za treniranje, validaciju i test. Grupa podataka za treniranje se koristi za obučavanje modela, grupa podataka za validaciju korisna je za podešavanje hiperparametara i evaluaciju performansi tokom treniranja, dok se test grupa podataka koristi za konačnu procenu modela nakon treniranja. Pravilna podela podataka je ključna za izbegavanje \todo{english}overfitting-a i osiguranje da model može dobro da generalizuje na novim, neviđenim podacima.
	
	\subsection{Kreiranje sintetičkog skupa podataka}
	\todo[inline]{Ovaj pasus treba da ide pre augmentacije i onda da nakon njega usledi prica o augmentaciji, jer i sintetički podaci prolaze proces augmentacije}
	\todo[inline]{Priloži sintetičke slike(tablica bez teksta -> tablica sa tekstom) i način kako one nastaju: inicijalno se ukloni tekst sa nekoliko pravih tablica na koje se kasnije doda generisani tekst po pravilima za Srpske tablice. Takođe priloži i kod za generisanje teksta?}
	Kreiranje sintetičkog skupa podataka pomaže uećanju inicijalnog skupa i posebno je od značaja kada je dostupnost realnih podataka ograničena.
